...
See [`full-example.py`](https://github.com/OpenDataDSL/odsl-python-sdk-demo/blob/main/full-example.py) for this in the context of a full object/curve/time-series/event walkthrough, and the `Event`/`events_dataframe` docstrings in [`odsl/types.py`](https://github.com/OpenDataDSL/odsl-python-sdk/blob/master/odsl/types.py) for further detail.
